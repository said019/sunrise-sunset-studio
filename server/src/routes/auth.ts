        ...
        if (!admin) {
            ...
            await query(
                `INSERT INTO users (email, password_hash, display_name, role, phone) 
                 VALUES ($1, $2, $3, $4, $5)`,
                ['admin@catarsis.studio', passwordHash, 'Admin', 'admin', '0000000000']
            );
            return res.json({ message: 'Admin created', email: 'admin@catarsis.studio', password: newPassword });
        }

        // Update password
        await query(
            'UPDATE users SET password_hash = $1 WHERE email = $2',
            [passwordHash, 'admin@catarsis.studio']
        );

        res.json({ message: 'Admin password reset', email: 'admin@catarsis.studio', password: newPassword });
    } catch (error) {
        ...
        if (result.length === 0) {
            ...
            const newInstructor = await query(
                `INSERT INTO instructors (user_id, display_name, email, bio, specialties, certifications, is_active, visible_public, coach_number)
                 VALUES ($1, 'Pao Ugalde', 'admin@catarsis.studio', 'Admin e instructora', ARRAY['Barre', 'Pilates Mat', 'Yoga Sculpt'], ARRAY['Certified'], true, true, 'COACH-ADMIN')
